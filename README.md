...
- 运行:GitHub Actions 定时任务(北京时间每交易日 10:00),无需服务器,公共仓库零成本
...
之后每个交易日(北京时间约 09:37 首跑,日内每半小时复检) 自动运行;**没有信号就不打扰**,有跨越阈值或档位加深时
你会在微信/邮箱收到提醒。
...
| 指数 | 来源 | 口径 |
|---|---|---|
| 沪深300 / 中证500 / 标普500 / 纳斯达克100 | 蛋卷指数估值接口(同一接口) | PE 近 10 年分位,四指数同源同窗口 |

> 注:蛋卷估值详情页对不同指数可能展示其他统计窗口(近5年/全部),数值与本系统略有差异属正常;
> 本系统统一采用估值列表接口口径,保证四指数横向可比。multpl(标普500月度PE)与
> Nasdaq(价格分位)适配器保留在代码中作为备用源,主源失败时可在 `config/indices.yaml` 切换。

历史分位不是绝对安全线,低估可能持续很久,本系统所有输出仅为提醒参考,不构成投资建议。
